...
for dpf in dirs:
	...
	#Lcd size
	keys = re.findall(r'#[ \t]*define[ \t]+LCD_([0-9]+x[0-9]+)$', data, re.MULTILINE)
	if len(keys) != 0:
...
alltypes = sorted(alltypes, key=lambda k: (k['Status'], k['Lcd_size'], k['Type']))
...
for typedict in alltypes:
	if laststatus != typedict['Status']:
		...
		if laststatus == 'supported':
			...
			kt.write('\n\n#**************** UNSUPPORTED DPFS **************************\n')
	if lastsize != typedict['Lcd_size']:
		lastsize = typedict['Lcd_size']
		kt.write('\n###################### %s ##############################\n' % lastsize)
	kt.write('\n#')
...
print "%d types written to knowntypes.py" % dpf_ok,
if dpf_ok < dpf_count:
